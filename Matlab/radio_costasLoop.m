...
mixedI = zeros(size(fmdemod));
mixedQ = zeros(size(fmdemod));
...
for n=2:length(fmdemod) 
	...
	%mixing
	mixedQ(n) = fmdemodFiltered(n) * real(vco(n));
	mixedI(n) = fmdemodFiltered(n) * imag(vco(n));

	%Matched Filter
	[mixedI(n), filterStateMatchedI] = filter(bMatched, 1, mixedI(n), filterStateMatchedI);
	[mixedQ(n), filterStateMatchedQ] = filter(bMatched, 1, mixedQ(n), filterStateMatchedQ);

	%phase detector (multiplier)
	phaseDiff(n) = mixedI(n) * mixedQ(n);
	...
	if(mixedI(n-1) > 0 && mixedI(n) < 0) || (mixedI(n-1) < 0 && mixedI(n) > 0)
	...
	if mod(timeAfterZeroCrossing, bitDur) == ceil(bitDur/2)
		...
		samplePoints(n) = mixedI(n);
		biphasesymbols(biphaseindex, k, phSteps) = mixedI(n) + 1i*mixedQ(n);
...
if draw == 1
	...
	figure
	plot(phiIncHistory, 'b');
	title('phiIncHistory');

	Y_phaseDiff = fft(phaseDiff);
	Y_phaseDiff = abs(Y_phaseDiff(1:ceil(length(Y_phaseDiff)/2)));
	f = fs*(0:length(Y_phaseDiff)-1)/(2*length(Y_phaseDiff));
	figure
	plot(f, Y_phaseDiff, 'b');
	title('spectrum of phaseDiff');
	...
	Y_fmdemodFiltered = fft(fmdemodFiltered);
	Y_fmdemodFiltered = abs(Y_fmdemodFiltered(1:ceil(length(Y_fmdemodFiltered)/2)));
	f = fs*(0:length(Y_fmdemodFiltered)-1)/(2*length(Y_fmdemodFiltered));
	figure
	plot(f, Y_fmdemodFiltered, 'b');
	title('spectrum of fmdemodFiltered');
