...
AUDIO_ONLY = 0;		%setting to 1 skips the RDS part

debugread=1;
if debugread==0
fileID = fopen('ethstream_neu.bin');
...
beforeFilter = mixedsignal99_9MHz;
...
%xhistDebug = zeros(length(b), length(mixedsignal99_9MHz));
%yhistDebug = zeros(length(a), length(mixedsignal99_9MHz));
%for index=1:length(mixedsignal99_9MHz)
%    xhist=circshift(xhist,[1,0]);
%    xhist(1)=mixedsignal99_9MHz(index);
...
%	xhistDebug(1:length(xhist), index) = xhist;
%	yhistDebug(1:length(yhist), index) = yhist;
%end
...
fmdemod = 0.002*imag(conj(decisignal(1:end-1)).*decisignal(2:end));
...
clear e f n fs phd_output phi_hat sampleCounter ki kp a b
clear startOfSymbol t symbCurSign symbOldSign symbolRate h phase
clear sampleDur vcoRiseEdgeCounter bitDur bitDurInVcoEdges
clear bitRate index timeAfterZeroCrossing fmdemod xhist
