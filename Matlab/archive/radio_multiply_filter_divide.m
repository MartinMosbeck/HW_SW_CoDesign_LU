...
mixedsignal = zeros(length(fmdemod),1);
...
for n=2:length(fmdemod) 
	...
	%mixing
	mixedsignal(n) = subCarrier(n) * vco(n);

	%Matched Filter
	[mixedsignal(n), filterStateMatched] = filter(bMatched, 1, mixedsignal(n, k, phSteps), filterStateMatched);
